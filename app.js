document.addEventListener('DOMContentLoaded', function() {
    ...
    let currentPage = 1;
    let totalPages = 1;
    let currentCategory = ''; // Categoria padrão vazia para pegar todas
    let filters = {};
    ...
    function buscarProdutosPorCategoria(page = 1) {
        ...
        // Adicionando o termo da categoria se for selecionado
        if (currentCategory === 'ferramentas') {
            // Buscar apenas produtos da categoria Ferramentas
            apiUrl += `&category=MLB1246`; // Código da categoria "Ferramentas" no Mercado Livre
        } else if (currentCategory === 'envios-full') {
            // Buscar apenas produtos com Envios Full (envio rápido)
            apiUrl += `&shipping=fulfillment`;
        ...
        // Adicionando filtros de marca
        if (filters.brand && filters.brand.length > 0) {
            const brandFilter = filters.brand.map(brand => `BRAND=${brand}`).join('&');
            apiUrl += `&${brandFilter}`;
        }
    ...
    applyFiltersButton.addEventListener('click', function() {
        ...
        filters.brand = selectedBrands.length > 0 ? selectedBrands : [];
